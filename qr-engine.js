/**
 * Lightweight Standalone QR Code Generator for NIAT Entry Pass Verification
 * Pure JavaScript, zero external dependencies, robust Error Correction Level M.
 */
(function (global) {
  // QR Code Model 2 implementation (byte mode)
  ...
  var QRUtil = {
    PATTERN_POSITION_TABLE: [
      ...
      [6, 28, 50, 72], [6, 30, 54, 78], [6, 32, 58, 84], [6, 34, 62, 90], [6, 28, 50, 72, 94]
    ],
  ...
  // High-level render function to HTML5 Canvas
  global.drawQRCodeToCanvas = function (canvas, text, options) {
    options = options || {};
    var qr = new QRCodeModel(options.typeNumber || 0, options.errorCorrectLevel || QRErrorCorrectLevel.M);
    qr.addData(text);
    qr.make();
    ...
    ctx.fillStyle = options.fgColor || '#0B1730';
    for (var row = 0; row < moduleCount; row++) {
      for (var col = 0; col < moduleCount; col++) {
